...
def main(
    ...
    *,
    ...
    # Delays
    # =========
    # Align delays to the block-duration raster to avoid timing assertion
    # errors when writing the sequence.
    bd_raster = system.block_duration_raster

    delay_TE = TE - pp.calc_duration(rf) / 2 - pp.calc_duration(gx_pre) - pp.calc_duration(gx) / 2
    delay_TE = np.round(delay_TE / bd_raster) * bd_raster
    ...
    delay_TR = np.round(delay_TR / bd_raster) * bd_raster

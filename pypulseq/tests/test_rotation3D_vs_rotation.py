...
def compare_gradient_sets(grad_set_A: list[SimpleNamespace], grad_set_B: list[SimpleNamespace], tolerance: float = 0) -> bool:
    ...
    -------
    is_equal : bool
        True if the two sets of gradients are equal with the tollerance.
    """
    channel_list = ['x', 'y', 'z']
    ...
    def check_gradient_set_and_get_channel_grad_dict(gradient_set):
        ...
        for grad in gradient_set:
            ...
            assert grad.channel not in channel_grad_dict, 'There must not be two gradients with the same channel in each set.'
    ...
    def compare_gradients(grad_A, grad_B):
        ...
        for key in grad_A_dict.keys():
            if not key in grad_B_dict.keys():
                return False
            elif type(grad_A_dict[key]) == float and type(grad_B_dict[key]) == float:
                if grad_A_dict[key] - grad_B_dict[key] > tolerance:
                    return False
            elif type(grad_A_dict[key]) == np.float64 and type(grad_B_dict[key]) == np.float64:
                if grad_A_dict[key] - grad_B_dict[key] > tolerance:
                    return False
...
def test_rotation3D_vs_rotation():
    ...
    for angle_degree in [0.0, 0.1, 1, 2, 3, 60, 90, 180, 360] + list(range(10, 450, 30)) + [-0.1, -1, -90, -180, -360, -400]:
        ...
        for rotation_axis in channel_list:
            ...
            for grad in grad_list:
                ...
                assert compare_gradient_sets(grads_rotated, grads_rotated3D, tolerance=1e-6), 'Result of rotate and rotate3D should be the same!'
                assert compare_gradient_sets(grads_rotated_double, grads_rotated3D_double, tolerance=1e-6), 'Result of double rotate and rotate3D should be the same!'
                assert compare_gradient_sets(grads_rotated_double, grads_rotated3D_double_2, tolerance=1e-6), 'Result of second double rotate and rotate3D should be the same!'

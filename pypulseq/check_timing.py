...
def check_timing(seq: Sequence) -> Tuple[bool, List[SimpleNamespace]]:
    ...
    def div_check(a: float, b: float, event: str, field: str, raster: str):
        ...
        if not is_ok:
            error_report.append(SimpleNamespace(block=block_counter, event=event, field=field, value=a, value_rounded=c_rounded*b, error=(a - c_rounded*b), raster=raster, error_type='RASTER'))
    ...
    for block_counter in seq.block_events:
        ...
        div_check(duration, seq.system.block_duration_raster, event='block', field='duration', raster='block_duration_raster')

        if abs(duration - seq.block_durations[block_counter]) > eps:
            error_report.append(SimpleNamespace(block=block_counter, event='block', field='duration', error_type='BLOCK_DURATION_MISMATCH', value=duration, duration=seq.block_durations[block_counter]))
        ...
        for event, e in block.__dict__.items():
            ...
            if hasattr(e, "delay"):
                if e.delay < -eps:
                    error_report.append(SimpleNamespace(block=block_counter, event=event, field='delay', error_type='NEGATIVE_DELAY', value=e.delay))
            ...
            if hasattr(e, "type") and e.type == "trap":
                div_check(e.rise_time, seq.system.grad_raster_time, event=event, field='rise_time', raster='grad_raster_time')
                div_check(e.flat_time, seq.system.grad_raster_time, event=event, field='flat_time', raster='grad_raster_time')
                div_check(e.fall_time, seq.system.grad_raster_time, event=event, field='fall_time', raster='grad_raster_time')

        # Check RF dead times
        if block.rf is not None:
            if block.rf.delay - block.rf.dead_time < -eps:
                error_report.append(SimpleNamespace(block=block_counter, event='rf', field='delay', error_type='RF_DEAD_TIME', value=block.rf.delay, dead_time=block.rf.dead_time))

            if (
                block.rf.delay + block.rf.t[-1] + block.rf.ringdown_time - duration
                > eps
            ):
                error_report.append(SimpleNamespace(block=block_counter, event='rf', field='duration', error_type='RF_RINGDOWN_TIME', value=block.rf.delay + block.rf.t[-1], duration=duration, ringdown_time=block.rf.ringdown_time))

        # Check ADC dead times
        if block.adc is not None:
            if block.adc.delay - seq.system.adc_dead_time < -eps:
                error_report.append(SimpleNamespace(block=block_counter, event='adc', field='delay', error_type='ADC_DEAD_TIME', value=block.adc.delay, dead_time=seq.system.adc_dead_time))

            adc_end = block.adc.delay + block.adc.num_samples * block.adc.dwell + seq.system.adc_dead_time

            if (adc_end > duration + eps):
                error_report.append(SimpleNamespace(block=block_counter, event='adc', field='duration', error_type='POST_ADC_DEAD_TIME', value=block.adc.delay + block.adc.num_samples * block.adc.dwell, duration=duration, dead_time=seq.system.adc_dead_time))
...
def print_error_report(seq: Sequence,
                       error_report: List[SimpleNamespace],

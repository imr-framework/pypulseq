...
def sig_2_seq(
    ...
    """
    Creates a radio-frequency sinc pulse event using the sigpy rf pulse library and optionally accompanying slice
    select, slice select rephasing trapezoidal gradient events.
    ...
    if np.sum(rf_freq) == 0:
        # Step 1: Get signal, t
        [signal, t] = get_signal(
            pulse=pulse, duration=duration, flip_angle=flip_angle, system=system
        )
...
def disp_pulse(am_pulse: float, fm_pulse: float = 0, tb=4, duration=3e-3):
    if np.sum(fm_pulse) == 0:
        ...
        [a, b] = rf_ext.sim.abrm(am_pulse, x, True)
        ...
        for ii in range(np.size(b1)):
            [a[ii], b[ii]] = rf_ext.sim.abrm_nd(
                2 * np.pi * (T / t.shape[0]) * 4258 * b1[ii] * am_pulse,
                np.ones(1),
...
def get_RF_train(am_pulse, fm_pulse, duration, system, flip_angle):
    ...
    [signal, _] = get_signal(
        pulse=am_pulse, duration=duration, flip_angle=flip_angle, system=system
    )

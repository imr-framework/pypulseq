...
def rotate(*args: SimpleNamespace, angle: float, axis: str, system: Union[Opts, None] = None) -> List[SimpleNamespace]:
    ...
    There is no change in behaviour of rotate() for rotations around the x- or z-axis.
    ...
    if axis == 'y':
        ...
        warning_message += 'There is no change in behaviour of rotate() for rotations around the x- or z-axis.'
        warn(warning_message, stacklevel=2)
        axes_to_rotate = [axes_to_rotate[1], axes_to_rotate[0]]  # reverse the list to preserve the correct handiness of the rotation matrix

...
from pypulseq.supported_labels_rf_use import get_supported_labels, get_supported_rf_use
...
def write(self, file_name: Union[str, Path], create_signature, remove_duplicates=True) -> Union[str, None]:
    ...
    with open(file_name, 'w') as output_file:
        ...
        # different to one, loaded (and stored in the seq object)
        output_file.write('[VERSION]\n')
        output_file.write(f'major {version_major}\n')
        output_file.write(f'minor {version_minor}\n')
        output_file.write(f'revision {version_revision}\n')
        output_file.write('\n')
        ...
        if len(self.rf_library.data) != 0:
            ...
            output_file.write(f'# Field "use" is the initial of: {" ".join(get_supported_rf_use()).strip()}\n')
        ...
        if len(self.adc_library.data) != 0:
            ...
            for k in self.adc_library.data:
                data = np.multiply(self.adc_library.data[k], [1, 1e9, 1e6, 1, 1, 1, 1])
                s = id_format_str.format(k, *data)
                output_file.write(s)
            output_file.write('\n')

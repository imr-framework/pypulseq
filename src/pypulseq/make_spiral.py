...
def make_spiral(
    ...
    while r0 < rmax_inv_cm:
        ...
        # findq2r2
        q2, r2, slew_min = _find_q2_r2(
            smax_cur, gmax_cur, r0, r1, to, adc_raster_time, N_leaves, f_coeff, rmax_inv_cm, smax_hw, gamma_hz_g
        )
        ...
        if resonances and r0 > 1e-6:
            ...
            for min_f, max_f in resonances:
                if min_f <= freq <= max_f:
                    ...
                    dg_max = _find_delta_g(
                        slew_margin * smax_hw, r0, r1, to, N_leaves, f_coeff, rmax_inv_cm, gamma_hz_g
                    )
